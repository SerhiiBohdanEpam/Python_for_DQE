    ...
    def add(self):
        ...
        if method.lower() == 'file':  # If user chooses to enter data via file
            ...
            for item in file_contents:
                ...
                if item.get('name') == 'news':
                    self.text = item.find('text').text
                    self.city = item.find('city').text
    ...
    def add(self):
        ...
        if method.lower() == 'file':  # If user chooses to enter data via file
            ...
            for item in file_contents:
                ...
                if item.get('name') == 'ad':
                    self.text = item.find('text').text
                    self.expiration_date = datetime.datetime.strptime(item.find('date').text, '%Y-%m-%d')
        else:  # If user chooses to enter data manually
            # Request data from user
        ...
        if method.lower() == 'file':  # If user chooses to enter data via file
            ...
            for item in file_contents:
                ...
                if item.get('name') == 'product':
                    ...
                    self.price = item.find('price').text
                    self.receipt_date = datetime.datetime.strptime(item.find('date').text, '%Y-%m-%d')
        else:  # If user chooses to enter data manually
            # Request data from user
...
content.publish("my_content_feed_from_xml.txt")
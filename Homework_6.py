...
def add_info_from_file(filename):
    with open(filename, 'r') as f:
        return f.read().splitlines()  # Returns a list of lines in file
...
class News:
    ...
    def add(self):
        ...
        if method.lower() == 'file':  # If user chooses to enter data via file
            ...
            # Parse lines from file contents for attributes
            self.text = correcter(file_contents[0])
            self.city = correcter(file_contents[1])
...
class NewProduct:
    ...
    def add(self):
        ...
        if method.lower() == 'file':  # If user chooses to enter data via file
            ...
            file_contents = add_info_from_file(filename)
            # Parse lines from file contents for attributes
            self.name = correcter(file_contents[0])
